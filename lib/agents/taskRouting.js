...
const buildRoleLockFromAgent = ({ agent, runnerKey }) => {
  ...
  const definition = getRoleDefinition(dbRoleKey || fallbackRoleKey);
  const allowedFromDb = parseAllowedTaskTypes(agent?.allowedTaskTypes);
  const allowedTaskTypes = allowedFromDb.length > 0 ? allowedFromDb : definition.allowedTaskTypes.slice();
  const defaultMode = normalizeMode(agent?.defaultMode, definition.defaultMode);
  const webBudgetFromDb =
    Number.isFinite(Number(agent?.webBudget)) && Number(agent?.webBudget) >= 0
      ? Number(agent.webBudget)
      : definition.webBudget;
  const escalationPolicy = toText(agent?.escalationPolicy) || definition.escalationPolicy;
  const maxOutputItems =
    agent?.maxOutputItems && typeof agent.maxOutputItems === "object"
...
const assignRoleLocksForAgents = (agents) => {
  ...
  list.forEach((agent) => {
    ROLE_KEYS.forEach((roleKey) => {
      pairs.push({
        agentId: agent.id,
        roleKey,
        score: scoreRoleMatch({ agent, roleKey })
      });
    });
  });
  ...
  for (const pair of pairs) {
    if (assignedAgents.has(pair.agentId)) continue;
    if (assignedRoles.has(pair.roleKey)) continue;
    if (pair.score <= 0) continue;
    assignedAgents.add(pair.agentId);
    assignedRoles.add(pair.roleKey);
    resultByAgent.set(pair.agentId, pair.roleKey);
  }

  const remainingRoles = ROLE_KEYS.filter((roleKey) => !assignedRoles.has(roleKey));
  for (const agent of list) {
    if (assignedAgents.has(agent.id)) continue;
    const roleKey = remainingRoles.shift() || DEFAULT_ROLE.roleKey;
    assignedAgents.add(agent.id);
    assignedRoles.add(roleKey);
    resultByAgent.set(agent.id, roleKey);
  }

  return list.map((agent) => {
    const roleKey = resultByAgent.get(agent.id) || DEFAULT_ROLE.roleKey;
