...
const collectNextSteps = (payload, taskType) => {
  ...
  if (taskType === "competitor_search_fast") {
    return [
      "Выберите 5 конкурентов из списка и проверьте, кто пересекается с вашим ICP.",
      "Добавьте ваш URL и гео, чтобы я сузил выдачу до прямых конкурентов в вашей нише.",
      "Запустите следующий шаг: короткое сравнение «почему выбирают их vs вас»."
    ];
  }
  return [
...
const COMPETITOR_SUGGESTION_LIBRARY = [
  {
    patterns: [/пульс/i, /опрос/i, /enps/i, /вовлеч/i, /wellbeing/i, /стресс/i],
    items: [
      { name: "Leapsome", url: "https://www.leapsome.com/", why: "Пульс-опросы, вовлеченность, performance циклы." },
      { name: "Culture Amp", url: "https://www.cultureamp.com/", why: "Employee experience и регулярные опросы команд." },
      { name: "Workday Peakon", url: "https://www.workday.com/en-us/products/employee-voice.html", why: "Платформа employee voice с pulse survey фокусом." },
      { name: "Lattice", url: "https://lattice.com/", why: "Оценка вовлеченности и performance-процессы." },
      { name: "15Five", url: "https://www.15five.com/", why: "Пульс-чек-ины и управление вовлеченностью." },
      { name: "Officevibe", url: "https://officevibe.com/", why: "Регулярные pulse-опросы и eNPS." }
    ]
  },
  {
    patterns: [/crm/i, /bitrix|битрикс/i, /amocrm/i, /лид/i],
    items: [
      { name: "amoCRM", url: "https://www.amocrm.ru/", why: "CRM для лидов и воронки продаж." },
      { name: "Битрикс24", url: "https://www.bitrix24.ru/", why: "CRM + автоматизация коммуникаций и задач." },
      { name: "HubSpot CRM", url: "https://www.hubspot.com/products/crm", why: "CRM с маркетинг/продажи автоматизацией." },
      { name: "Pipedrive", url: "https://www.pipedrive.com/", why: "Управление пайплайном и продажами." },
      { name: "Freshsales", url: "https://www.freshworks.com/crm/sales/", why: "CRM для среднего бизнеса с авто-сценариями." }
    ]
  },
  {
    patterns: [/ai|ии/i, /автоматизац/i, /агент/i],
    items: [
      { name: "Salesforge", url: "https://salesforge.ai/", why: "AI-аутрич и автоматизация продаж." },
      { name: "Apollo", url: "https://www.apollo.io/", why: "Лидоген + outbound workflow." },
      { name: "Clay", url: "https://www.clay.com/", why: "Обогащение лидов и автоматизация prospecting." },
      { name: "Instantly", url: "https://instantly.ai/", why: "Outbound email-автоматизация." },
      { name: "Lemlist", url: "https://www.lemlist.com/", why: "Мультиканальный аутрич и sequence." }
    ]
  }
];

const pickBestEffortCompetitorsFromPrompt = (userPrompt) => {
  const text = sanitizeOutputText(userPrompt, 1600).toLowerCase();
  const matches = COMPETITOR_SUGGESTION_LIBRARY.filter((entry) =>
    entry.patterns.some((pattern) => pattern.test(text))
  );
  const items = matches.length > 0 ? matches.flatMap((entry) => entry.items) : COMPETITOR_SUGGESTION_LIBRARY[0].items;
  const unique = [];
  const seen = new Set();
  for (const item of items) {
    const key = sanitizeOutputText(`${item.name}|${item.url}`.toLowerCase(), 220);
...
const renderCompetitorFastMain = (payload, userPrompt = "") => {
  ...
  if (competitors.length === 0) {
    const suggestions = pickBestEffortCompetitorsFromPrompt(userPrompt);
    if (suggestions.length === 0) {
      return "Нужен URL сайта или 1–2 ключевых слова ниши, чтобы собрать 5–8 конкурентов.";
    }
    return suggestions
      .map(
        (item, index) =>
          `${index + 1}. ${sanitizeOutputText(item.name, 140)} — ${sanitizeOutputText(item.url, 260)}\n${sanitizeOutputText(item.why, 220)}`
      )
      .join("\n\n");
  }
  return competitors
...
const renderLeadScoringMain = ({ payload }) => {
  const safe = payload && typeof payload === "object" ? payload : {};
  const meta = safe.meta && typeof safe.meta === "object" ? safe.meta : {};
  const leads = Array.isArray(safe.hot_leads)
    ? safe.hot_leads
        .map((item) => (item && typeof item === "object" ? item : null))
        .filter(Boolean)
        .slice(0, 10)
  ...
  if (leads.length > 0) {
    return leads
      .map((item, index) => {
        ...
        const url = normalizeUrlFromText(item.url || "");
        const why =
          sanitizeOutputText(item.why_match || item.why_now || item.evidence || "", 220) ||
          "Совпадает с критериями отбора.";
        const confidence = Number(item.confidence || item.hot_score || 0);
        const leadType = sanitizeOutputText(item.lead_type || "", 32) || "Warm";
        return `${index + 1}. ${title}${url ? ` — ${url}` : ""}\n${why} | Confidence: ${Math.max(
          0,
          Math.min(100, Number.isFinite(confidence) ? Math.round(confidence) : 0)
        )} | ${leadType}`;
      })
      .join("\n\n");
  }
  ...
  const lines = [];
  const topSegments = Array.isArray(meta.top_best_segments)
    ? meta.top_best_segments
        .map((item) => (item && typeof item === "object" ? item : null))
        .filter(Boolean)
        .slice(0, 10)
  ...
  const warmTargets = Array.isArray(meta.warm_targets)
    ? meta.warm_targets
        ...
        .slice(0, 12)
    : [];
  const manualQueries = Array.isArray(meta?.manual_hot_hunting?.queries)
    ? meta.manual_hot_hunting.queries
        .map((item) => sanitizeOutputText(item, 180))
        .filter(Boolean)
        .slice(0, 8)
    : [];

  if (topSegments.length > 0) {
    lines.push("Top сегменты для приоритета:");
    topSegments.forEach((segment, index) => {
      ...
      const why = sanitizeOutputText(
        segment.pain_trigger || segment.why_need || segment.why_now || segment.why_fit_now,
        180
      );
      if (name) lines.push(`${index + 1}. ${name}${why ? ` — ${why}` : ""}`);
    });
  }

  if (warmTargets.length > 0) {
    if (lines.length > 0) lines.push("");
    lines.push("Warm targets (без веб-поиска):");
    warmTargets.forEach((target, index) => {
      const title =
        sanitizeOutputText(
          target.company_type ||
            target.segment ||
            target.company_or_type ||
            target.title ||
            target.niche ||
            "",
          180
        ...
        `Сегмент ${index + 1}`;
      const why = sanitizeOutputText(
        target.pain_trigger || target.why_need || target.why_fit_now || target.why_fit || target.trigger,
        180
      );
      lines.push(`${index + 1}. ${title}${why ? ` — ${why}` : ""}`);
    });
  }

  if (manualQueries.length > 0) {
    if (lines.length > 0) lines.push("");
    lines.push("Manual hunting запросы:");
    manualQueries.forEach((query, index) => {
      lines.push(`${index + 1}. ${query}`);
    });
  }

  if (lines.length === 0) {
    return "Сигналы не найдены, но можно продолжить с ручным списком компаний для ранжирования.";
  }
...
const buildUnifiedDocument = ({ payload, rawText, context = {} }) => {
  ...
  const genericSummaryPhrases = new Set([
    "Ответ подготовлен по запросу пользователя.",
    "Собраны ключевые наблюдения и рекомендации.",
    "Ниже — полный результат без служебной телеметрии.",
    "Данные обработаны без сокращений."
  ]);
  const primarySummary = summaryLines[0] || "";
  const summaryByTask =
    taskType === "competitor_search_fast"
      ? "Подготовил список конкурентов по вашей нише."
      : taskType === "company_analysis"
        ? "Подготовил разбор сайта и ключевые точки улучшения."
        : taskType === "outreach_copy"
          ? "Подготовил готовые тексты для отправки."
          : "Готово.";
  const summary =
    !primarySummary || genericSummaryPhrases.has(primarySummary)
      ? summaryByTask
      : primarySummary;
  ...
  if (taskType === "company_analysis" && payload && typeof payload === "object") {
    ...
    resultText = renderCompetitorFastMain(payload, context.userPrompt || "");
  } else if (taskType === "lead_scoring" && payload && typeof payload === "object") {
    resultText = renderLeadScoringMain({ payload });
  } else if (
